...
export default function ManualCollection() {
  ...
  useEffect(() => {
    ...
    const triggerFeed = event => {
      if (lock.current) return;
      const { feedId, url: feedUrl, name, filterKeywords } = event.detail || {};
      if (!feedId) return;
      setUrl(feedUrl || '');
      run(async () => {
    ...
    window.addEventListener('trigger-feed-collection', triggerFeed);
    return () => {
      window.removeEventListener('choose-collection-source', choose);
      window.removeEventListener('trigger-feed-collection', triggerFeed);
  ...
  return <section id="manual-collection" className="panel manual-collection">
    ...
    <fieldset className="collection-input" disabled={busy}>
      ...
      <button className="button button--primary" type="button" onClick={()=>run(async()=>{ setDrafts([]); setSelected([]); setErrors([]); setStatus('正在检查站点规则并读取RSS内容，请等待…'); const d=await call({action:'rss',url}); setDrafts(d.drafts); setSelected(d.drafts.map(x=>x.id)); setErrors(d.errors || []); setStatus(`读取结束：${d.drafts.length}篇可预览，${d.requests}次网页请求${d.cached?'，已复用缓存':''}。尚未调用AI、尚未入库。`); })}>手动读取此链接</button>
    </fieldset>
    ...
    {drafts.length>0 && <>
      <div style={{display:'flex',alignItems:'center',gap:10,flexWrap:'wrap',marginTop:4}}>
        <button className="button button--primary" disabled={busy||!configured||!selected.length} type="button" onClick={()=>run(async()=>{let count=0; for(const d of drafts.filter(x=>selected.includes(x.id))){setStatus(`DeepSeek正在分析第${++count} / ${selected.length}篇：${d.title}`); const result=await call({action:'analyze',id:d.id});replace(result.draft);}setStatus('分析完成。请对照原文检查结果，再点击“确认入库”。');})}>分析所选 {selected.length} 篇</button>
        {drafts.some(d => d.analysis && !d.articleId) && (
          <button className="button button--secondary" disabled={busy} type="button" onClick={()=>run(async()=>{
            setStatus('正在批量入库已完成分析的文章…');
            ...
            setStatus('批量入库完成。已加入本地合规资讯库。');
          })}>批量入库已分析内容</button>
        )}
        {!configured&&<span style={{color:'var(--color-text-muted)',fontSize:12}}>请先配置服务端API Key。</span>}
      </div>
      <div className="collection-previews">{drafts.map(d=><article className="collection-preview" key={d.id}>
        ...
        <div className="collection-compare"><section><h3>原文摘录</h3><div className="collection-original">{d.text}</div>{d.kind!=='paste'&&<a href={d.url} target="_blank" rel="noopener noreferrer">打开原文</a>}</section><section><h3>DeepSeek分析</h3>{d.analysis?<>{d.analysis.isRelevant === false && <div style={{background:'rgba(255,93,103,.15)',borderLeft:'3px solid #ff5d67',padding:'6px 10px',borderRadius:3,marginBottom:8,fontSize:12,color:'#ffadb3'}}>[研判警示] DeepSeek研判：该内容与 AI 安全合规无关。</div>}<p>{d.analysis.summary}</p><p>{d.analysis.category} · {d.analysis.tags.join(' / ')}</p><h4>业务影响 · AI分析</h4><p>{d.analysis.impact}</p><h4>关注建议</h4><p>{d.analysis.action}</p><small>{d.analysis.model}</small></>:<p style={{color:'var(--color-text-muted)',fontSize:12}}>尚未分析。勾选文章后点击“分析所选”。</p>}</section></div>
        {d.articleId?<div style={{display:'flex',alignItems:'center',gap:10,flexWrap:'wrap',marginTop:4}}><span className="badge badge--success">已入库</span><a href={`/analysis?article=${d.articleId}`}>查看资讯库</a><a href="/morning">前往每日早报</a></div>:<button className="button button--primary" type="button" disabled={busy||!d.analysis} onClick={()=>run(async()=>{setStatus('正在保存到本地资讯库…');const result=await call({action:'save',id:d.id});replace({...d,articleId:result.articleId});setStatus('已入库。可前往每日AI早报生成早报。');})}>确认入库</button>}
      </article>)}</div>
    </>}
