...
export async function allowedRead(url, context, options = {}) {
  const u = normalizeUrl(url);
  const isFeedOrInternal = options.skipRobots || u.hostname === 'wewe.wilsongo.top' || u.hostname === 'rss.wilsongo.top' || u.hostname === 'radar.wilsongo.top' || u.pathname.includes('/feeds/') || u.pathname.startsWith('/api/rss/');
  if (!isFeedOrInternal) {
...
export function extractArticle(buffer, url, fallbackText = '') {
  ...
  if (rawDate) {
    try {
      const cleanDate = rawDate.includes('+') || rawDate.includes('Z') ? rawDate : (rawDate.includes(' ') ? rawDate.replace(' ', 'T') + '+08:00' : rawDate);
