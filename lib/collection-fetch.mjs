...
export async function allowedRead(url, context, options = {}) {
  const u = normalizeUrl(url);
  const isFeedOrInternal = options.skipRobots || u.hostname === 'wewe.wilsongo.top' || u.pathname.includes('/feeds/');
  if (!isFeedOrInternal) {
...
export function extractFeed(buffer, url, filterKeywords = null) {
  ...
  $('item,entry').each((_, el) => {
    ...
    try { const href = normalizeUrl(new URL(raw.trim(), url).href).href; if (!seen.has(href)) { seen.add(href); entries.push({ title, url: href, fallbackText }); } } catch {}
  });
