...
export async function allowedRead(url, context) {
  const u = normalizeUrl(url);
...
export function extractArticle(buffer, url) {
  ...
  const paragraphs = root.find('p,h1,h2,h3,li').map((_, el) => $(el).text().replace(/\s+/g, ' ').trim()).get().filter(Boolean);
...
export function extractFeed(buffer, url) {
  ...
  $('item,entry').each((_, el) => {
    const node = $(el), raw = node.find('link[rel="alternate"]').attr('href') || node.find('link').first().attr('href') || node.find('link').first().text();
    try { const href = normalizeUrl(new URL(raw.trim(), url).href).href; if (!seen.has(href)) { seen.add(href); entries.push({ title: node.find('title').first().text().trim(), url: href }); } } catch {}
  });
