...
export function listPendingDrafts(limit = 50) {
  const s = store(); if (!s) return [];
  return s.prepare('SELECT * FROM drafts WHERE analysis IS NULL ORDER BY id DESC LIMIT ?').all(limit).map(row => ({
    ...JSON.parse(row.body),
    id: row.id,
    analysis: null,
...
export const OFFICIAL_PRESET_FEEDS = [
  ...
  {
    id: 'preset-wechat-tc260',
    name: '全国网安标委',
    url: 'https://wewe.wilsongo.top/feed/MP_WXS_3922364654.xml',
    category: 'AI安全标准',
    description: '全国网络安全标准化技术委员会官方微信公众号（国家治理框架、标准实践指南与合规发布）',
    filterKeywords: '',
    enabled: 1,
    cadence: '每天'
  },
