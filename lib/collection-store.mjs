...
export function confirmDraft(id) {
  ...
  const a = d.analysis;
  const category = a.intelligenceType || a.category || '行业动态';
  ...
  const body = {
    title: d.title, source: d.source, sourceType: d.kind === 'paste' ? '用户提供' : '公开网页', url: d.kind === 'paste' ? '' : d.url,
    publishedAt: d.publishedAt || d.collectedAt, dateIsCollection: !d.publishedAt, collectedAt: d.collectedAt, originalExcerpt: d.text,
    summary: a.summary, intelligenceType: category, detailTag: a.detailTag || '', affectedEntity: a.affectedEntity || '', severity: a.severity || '',
    tags: Array.isArray(a.tags) ? a.tags : [], impact: a.impact || '', recommendedAction: a.action || '', insight: a.impact || '',
