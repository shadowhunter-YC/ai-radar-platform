...
export function filterReportArticles(articles, p = DEFAULT_PREFERENCES, now = Date.now()) {
  ...
  return articles.filter(a => {
    ...
    // 分类筛选 (若未设定或为空，则全选；命中重点博主或法案时豁免分类限制)
...
export function getRecommendedReportArticles(articles = [], p = DEFAULT_MORNING_PREFERENCES, now = Date.now()) {
  if (!Array.isArray(articles) || !articles.length) return [];
  // 1. 尝试当前偏好严格筛选
  let matched = filterReportArticles(articles, p, now);
  if (matched.length >= 1) return matched;

  // 2. 放宽分类与标签筛选（在当前时间跨度内全选）
  matched = filterReportArticles(articles, { ...p, types: [], tags: [] }, now);
  if (matched.length >= 1) return matched;

  // 3. 放宽至近 3 天（72小时）
  matched = filterReportArticles(articles, { ...p, period: '3d', types: [], tags: [] }, now);
  if (matched.length >= 1) return matched;

  // 4. 兜底返回全量最近资讯
  return articles.slice(0, p.limit || 20);
}
