export const DEFAULT_PREFERENCES = { types: [], tags: [], period: '24h', date: '', limit: 10 };
...
export function filterReportArticles(articles, p, now = Date.now()) {
  return articles.filter(a => {
    ...
    return (!p.types.length || p.types.includes(a.intelligenceType)) && (!p.tags.length || p.tags.some(tag => (a.tags || []).includes(tag) || normalizeTags(a.tags).includes(tag)));
