...
/**
 * 精准识别观点类与深度解读文章
 * 包含：显式标记为 opinion，或带有 深度解读/专家视角/行业观点 等细分标签或标签
 * @param {Object} article
 * @returns {boolean}
 */
export function isOpinionArticle(article) {
  if (!article) return false;
  if (article.contentNature === 'opinion') return true;
  const detail = String(article.detailTag || '').toLowerCase();
  if (
    detail.includes('深度解读') ||
    detail.includes('解读') ||
    detail.includes('专家') ||
    detail.includes('观点') ||
    detail.includes('评论') ||
  ...
  if (tags.some(t => {
    ...
    return s.includes('行业观点') || s.includes('深度解读') || s.includes('专家评论') || s.includes('观点评论');
  })) {
    return true;
  }
  return false;
